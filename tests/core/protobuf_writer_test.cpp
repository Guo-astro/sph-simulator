...
protected:
    ...
    bool read_snapshot(const std::string& filename, Snapshot& snapshot) const {
...
SCENARIO_WITH_FIXTURE(ProtobufWriterTestFixture, WritesEnergyData) {
    GIVEN("A Protobuf writer and test particle") {
        ...
        WHEN("We write energy data") {
            writer.write_energy(&particle, 1, 0.0);
    ...
    GIVEN("A 3D Protobuf writer") {
        ...
        WHEN("We write a 3D snapshot") {
            ...
            THEN("3D snapshot should be created and deserializable") {
                ...
                EXPECT_NEAR(pb_particle.pos_x(), 1.0, kTolerance);
                EXPECT_NEAR(pb_particle.pos_y(), 2.0, kTolerance);
                EXPECT_NEAR(pb_particle.pos_z(), 3.0, kTolerance);
            }
        }
