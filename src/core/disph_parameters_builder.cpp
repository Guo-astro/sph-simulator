...
AlgorithmParametersBuilder<DISPHTag>::AlgorithmParametersBuilder(std::shared_ptr<SPHParameters> base_params)
    : params(base_params) {
...
void AlgorithmParametersBuilder<DISPHTag>::validate_build() const {
    if (!has_artificial_viscosity) {
        throw std::runtime_error(
            "DISPH requires artificial viscosity. "
...
AlgorithmParametersBuilder<DISPHTag>& AlgorithmParametersBuilder<DISPHTag>::with_artificial_viscosity(
    real alpha,
...
AlgorithmParametersBuilder<DISPHTag>& AlgorithmParametersBuilder<DISPHTag>::with_artificial_conductivity(real alpha) {
    params->ac.is_valid = true;
    params->ac.alpha = alpha;
    return *this;
}

std::shared_ptr<SPHParameters> AlgorithmParametersBuilder<DISPHTag>::build() {
    validate_build();
    return params;
}

bool AlgorithmParametersBuilder<DISPHTag>::is_complete() const {
    return has_artificial_viscosity;
}

std::string AlgorithmParametersBuilder<DISPHTag>::get_missing_parameters() const {
    if (!has_artificial_viscosity) {
