 ...
 * @brief 2D Shock Tube DISPH Plugin
 *
 ...
 */
class DISPHShockTube2DPlugin : public SimulationPlugin<2> {
public:
    ...
    void initialize(std::shared_ptr<Simulation<2>> sim, std::shared_ptr<SPHParameters> param) override {
        ...
        // OPTIMIZED SETUP: Fewer particles, isotropic spacing
        // Strategy: Uniform mass particles, vary X-spacing to get density ratio
        ...
        // For cubic spline kernel in 2D: h ≈ kappa * dx
        // kappa ≈ 1.2 ensures kernel support covers appropriate neighbors
        ...
        if constexpr (USE_PERIODIC_BOUNDARY) {
            ...
            ghost_config = BoundaryBuilder<Dim>()
                               .in_range(Vector<Dim>{-0.5, 0.0}, Vector<Dim>{1.5, 0.5})
                               .with_mirror_in_dimension(0, MirrorType::FREE_SLIP, dx_left,
                                                         dx_right)  // X: frictionless shock tube walls
                               .with_mirror_in_dimension(1, MirrorType::NO_SLIP, dy,
                                                         dy)  // Y: sticky confined walls (uniform spacing)
                               .build();
        ...
        std::cout << "    Particle spacing: dy=" << dy << " (uniform)\n";
        ...
        std::cout << "  Generated " << sim->ghost_manager->get_ghost_count() << " ghost particles\n";
...
DEFINE_SIMULATION_PLUGIN(DISPHShockTube2DPlugin, 2)
